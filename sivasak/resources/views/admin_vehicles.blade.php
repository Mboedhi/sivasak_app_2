        ...
        <div class="table-container">
            <table>
                ...
                <tbody>
                    ...
                    @php
                    $vehicle_tax = \Carbon\Carbon::parse($vehicle->vehicle_tax);
                    $daysUntilTax = now()->diffInDays($vehicle->vehicle_tax);
                    $days = $vehicle_tax->diffForHumans(now());
                    @endphp
                    <tr
                        style="{{ $daysUntilTax <= 14 ? 'background-color: #dc3545 !important; color: white !important;' : '' }}">
                        <td style="{{ $daysUntilTax <= 14 ? 'color: white !important;' : '' }}">{{
                            $vehicle->vehicle_plate }}</td>
                        <td style="{{ $daysUntilTax <= 14 ? 'color: white !important;' : '' }}">{{ $vehicle->year }}
                        </td>
                        <td style="{{ $daysUntilTax <= 14 ? 'color: white !important;' : '' }}">{{
                            $vehicle->vehicle_type }}</td>
                        <td style="{{ $daysUntilTax <= 14 ? 'color: white !important;' : '' }}">{{
                        ...
                        <td class="{{ $daysUntilTax <= 14 ? 'text-danger fw-bold' : '' }}"
                            style="{{ $daysUntilTax <= 14 ? 'color: #ffc107 !important;' : '' }}">
                            {{ $vehicle->vehicle_tax}}
                            @if($daysUntilTax <= 14) <span class="badge bg-danger ms-2">Segera Jatuh Tempo ({{
                                $days }})</span>
                                @endif
                        </td>
                        <td style="{{ $daysUntilTax <= 14 ? 'color: white !important;' : '' }}">{{
                            $vehicle->head_cover_date}}</td>
                        <td style="{{ $daysUntilTax <= 14 ? 'color: white !important;' : '' }}">{{
                            $vehicle->tail_cover_date}}</td>
                        <td style="{{ $daysUntilTax <= 14 ? 'color: white !important;' : '' }}">{{ $vehicle->note}}</td>
                        <td style="{{ $daysUntilTax <= 14 ? 'color: white !important;' : '' }}">
                            <button class="edit-button"
                                onclick="window.location.href='{{ route('admin_editinputvehicle', ['vehicle_id' => $vehicle->vehicle_id]) }}'">Edit</button>
                            <button class="delete-button"
                                onclick="openDeleteModal({{ $vehicle->vehicle_id }})">Hapus</button>
